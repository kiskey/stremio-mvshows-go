// Version: 1.9.9
// Change log: Exposed ExtractInfohash and implemented ComputeMagnetSetHash for deterministic O(1) content fingerprinting based on sorted unique infohashes.
...
// ExtractInfohash exposes the internal infohash extraction logic for cross-package usage.
func ExtractInfohash(magnet string) string {
    return extractInfohash(magnet)
}

// ComputeMagnetSetHash generates a deterministic SHA256 hash from a set of magnet URIs.
// It extracts the infohash from each magnet, deduplicates them, sorts them, and hashes the result.
// This provides an O(1) comparison for thread content changes immune to tracker/dn format drift.
func ComputeMagnetSetHash(magnets []string) string {
    if len(magnets) == 0 {
        return ""
    }
