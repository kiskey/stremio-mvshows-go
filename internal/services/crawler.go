// Version: 1.4.0
// Change log: Standardized title extraction between RunCrawler and RunTargetedCrawler by introducing cleanScrapedTitle helper to strip site suffixes and normalize whitespace, preventing oscillating bypass failures.
...
// cleanScrapedTitle normalizes raw titles extracted from the forum by trimming whitespace,
// collapsing spaces, and removing known site suffixes that cause parsing divergence 
// between index crawling and targeted page crawling.
func cleanScrapedTitle(raw string) string {
    raw = strings.TrimSpace(raw)
    
    suffixes := []string{" - TamilMV", " - TamilBlasters", " - 1TamilMV", " - TamilMV.", " - TamilBlasters."}
...
func RunCrawler(cfg *config.Config, incremental bool) ([]CrawledThread, error) {
    ...
    c.OnHTML("h4.ipsDataItem_title > span.ipsType_break > a", func(e *colly.HTMLElement) {
        link := e.Attr("href")
        rawTitle := cleanScrapedTitle(e.Text) // Use unified cleaner
...
func RunTargetedCrawler(cfg *config.Config, threadURL, contentType, catalogID string) ([]CrawledThread, error) {
    ...
    c.OnHTML("a[href^=\"magnet:?\"]", func(e *colly.HTMLElement) {
        ...
        rawTitle = cleanScrapedTitle(rawTitle) // Use unified cleaner
        if rawTitle == "" {
            rawTitle = e.DOM.ParentsUntil("html").Find("title").Text()
            rawTitle = cleanScrapedTitle(rawTitle)
        }
