...
func main() {
    ...
    err = db.Update(func(tx *bolt.Tx) error {
        ...
        for _, m := range metadataToRewrite {
            _ = database.UpsertTmdbMetadata(tx, m) // Idempotent internal write
        }
        ...
        for targetNewHash, list := range duplicatesMap {
            ...
            for _, item := range list {
                ...
                for _, m := range item.MagnetURIs {
                    ...
                    ih := parser.ExtractInfohash(cleanM)
                    if cleanM != "" && ih != "" && !seenMags[ih] {
                        seenMags[ih] = true
            ...
            keptThread.MagnetSetHash = parser.ComputeMagnetSetHash(allMergedMags)
        ...
        // STEP 2.5: Normalize legacy magnet URIs and compute content fingerprints for all threads
        log.Println("Step 2.5: Normalizing legacy magnet URIs and computing content fingerprints...")
        ...
        for k, v := threadCursor.First(); k != nil; k, v = threadCursor.Next() {
            ...
            if errDec := database.DecodeGob(v, &t); errDec == nil {
                ...
                var normalizedMagnets []string
                for _, m := range t.MagnetURIs {
                    cleanM := parser.StripTrackersFromMagnet(m)
                    ih := parser.ExtractInfohash(cleanM)
                    if ih != "" && !seenInfohashes[ih] {
                        seenInfohashes[ih] = true
                        normalizedMagnets = append(normalizedMagnets, cleanM)
                    }
                }
                t.MagnetURIs = normalizedMagnets
                t.MagnetSetHash = parser.ComputeMagnetSetHash(normalizedMagnets)
            ...
            if errDec := database.DecodeGob(v, &t); errDec == nil {
                if t.Status == "linked" && t.TmdbID != nil {
                    ...
                    for _, magnet := range t.MagnetURIs {
                        ...
                        _ = database.UpsertMagnetCache(tx, parsedMagnet.Infohash, cleanMagnet)
